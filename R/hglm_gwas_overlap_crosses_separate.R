...
gwas_pen_bovans_load$pretty_name <- "bone strength"
gwas_pen_bovans_load$scan <- "PEN"
gwas_pen_bovans_load$cross <- "Bovans"

gwas_pen_lsl_load <- readRDS("outputs/gwas_pen_lsl_load_coord.Rds")
gwas_pen_lsl_load$pretty_name <- "bone strength"
gwas_pen_lsl_load$scan <- "PEN"
gwas_pen_lsl_load$cross <- "LSL"

gwas_cage_bovans_load <- readRDS("outputs/gwas_cage_bovans_load_coord.Rds")
gwas_cage_bovans_load$pretty_name <- "bone strength"
gwas_cage_bovans_load$scan <- "CAGE"
gwas_cage_bovans_load$cross <- "Bovans"

gwas_cage_lsl_load <- readRDS("outputs/gwas_cage_lsl_load_coord.Rds")
gwas_cage_lsl_load$pretty_name <- "bone strength"
gwas_cage_lsl_load$scan <- "CAGE"
gwas_cage_lsl_load$cross <- "LSL"

gwas_all_bovans_load <- readRDS("outputs/gwas_all_bovans_load_coord.Rds")
gwas_all_bovans_load$pretty_name <- "bone strength"
gwas_all_bovans_load$scan <- "JOINT"
gwas_all_bovans_load$cross <- "Bovans"

gwas_all_lsl_load <- readRDS("outputs/gwas_all_lsl_load_coord.Rds")
gwas_all_lsl_load$pretty_name <- "bone strength"
gwas_all_lsl_load$scan <- "JOINT"
gwas_all_lsl_load$cross <- "LSL"



## Body weight

gwas_pen_bovans_weight <- readRDS("outputs/gwas_pen_bovans_weight_coord.Rds")
gwas_pen_bovans_weight$pretty_name <- "body weight"
gwas_pen_bovans_weight$scan <- "PEN"
gwas_pen_bovans_weight$cross <- "Bovans"

gwas_pen_lsl_weight <- readRDS("outputs/gwas_pen_lsl_weight_coord.Rds")
gwas_pen_lsl_weight$pretty_name <- "body weight"
gwas_pen_lsl_weight$scan <- "PEN"
gwas_pen_lsl_weight$cross <- "LSL"

gwas_cage_bovans_weight <- readRDS("outputs/gwas_cage_bovans_weight_coord.Rds")
gwas_cage_bovans_weight$pretty_name <- "body weight"
gwas_cage_bovans_weight$scan <- "CAGE"
gwas_cage_bovans_weight$cross <- "Bovans"

gwas_cage_lsl_weight <- readRDS("outputs/gwas_cage_lsl_weight_coord.Rds")
gwas_cage_lsl_weight$pretty_name <- "body weight"
gwas_cage_lsl_weight$scan <- "CAGE"
gwas_cage_lsl_weight$cross <- "LSL"

gwas_all_bovans_weight <- readRDS("outputs/gwas_all_bovans_weight_coord.Rds")
gwas_all_bovans_weight$pretty_name <- "body weight"
gwas_all_bovans_weight$scan <- "JOINT"
gwas_all_bovans_weight$cross <- "Bovans"

gwas_all_lsl_weight <- readRDS("outputs/gwas_all_lsl_weight_coord.Rds")
gwas_all_lsl_weight$pretty_name <- "body weight"
gwas_all_lsl_weight$scan <- "JOINT"
gwas_all_lsl_weight$cross <- "LSL"


## pQCT & TGA

gwas_pen_bovans <- readRDS("outputs/gwas_pen_bovans_bone_phenotypes_coord.Rds")
gwas_cage_bovans <- readRDS("outputs/gwas_cage_bovans_bone_phenotypes_coord.Rds")
gwas_all_bovans <- readRDS("outputs/gwas_all_bovans_bone_phenotypes_coord.Rds")

gwas_pen_lsl <- readRDS("outputs/gwas_pen_lsl_bone_phenotypes_coord.Rds")
gwas_cage_lsl <- readRDS("outputs/gwas_cage_lsl_bone_phenotypes_coord.Rds")
gwas_all_lsl <- readRDS("outputs/gwas_all_lsl_bone_phenotypes_coord.Rds")


gwas_pen_bovans$scan <- "PEN"
gwas_pen_bovans$cross <- "Bovans"

gwas_pen_lsl$scan <- "PEN"
gwas_pen_lsl$cross <- "LSL"

gwas_cage_bovans$scan <- "CAGE"
gwas_cage_bovans$cross <- "Bovans"

gwas_cage_lsl$scan <- "CAGE"
gwas_cage_lsl$cross <- "LSL"

gwas_all_bovans$scan <- "JOINT"
gwas_all_bovans$cross <- "Bovans"

gwas_all_lsl$scan <- "JOINT"
gwas_all_lsl$cross <- "LSL"


cols <- c("pretty_name", "chr", "ps", "marker_id", "estimates", "LRT", "p", "scan", "cross")

combined <- rbind(gwas_pen_bovans_load[, cols],
                  gwas_pen_lsl_load[, cols],
                  gwas_cage_bovans_load[, cols],
                  gwas_cage_lsl_load[, cols],
                  gwas_all_bovans_load[, cols],
                  gwas_all_lsl_load[, cols],
                  gwas_pen_bovans_weight[, cols],
                  gwas_cage_bovans_weight[, cols],
                  gwas_pen_lsl_weight[, cols],
                  gwas_cage_lsl_weight[, cols],
                  gwas_all_bovans_weight[, cols],
                  gwas_all_lsl_weight[, cols],
                  gwas_pen_bovans[, cols],
                  gwas_pen_lsl[, cols],
                  gwas_cage_bovans[, cols],
                  gwas_cage_lsl[, cols],
                  gwas_all_bovans[, cols],
                  gwas_all_lsl[, cols])
...
write.csv(combined,
          "tables/supplementary_data_gwas_summary_stats_crosses_separate.csv",
          quote = FALSE,
          row.names = FALSE)
...
suggestive <- filter(combined, p < 1e-5)
...
write.csv(significant_table[, cols],
          file = "tables/significant_gwas_table_crosses_separate.csv",
          quote = TRUE,
          row.names = FALSE)

write.csv(suggestive_table[, cols],
          file = "tables/suggestive_gwas_table_crosses_separate.csv",
          quote = TRUE,
          row.names = FALSE)
